...
class Action_List_Helper:
    ...
    def remove_slot(self, index, remove_action=False, sync=True, cleanup=True):
        ...
        if total > 0:
            ...
            if self.check_index(index):
                ...
                if remove_action:
                    Utility_Function.remove_action_from_file(action, cleanup=cleanup)
                    # self.remove_action(action, cleanup=cleanup)
    ...
    def clear(self, remove_action=False):
        ...
        if remove_action:
            ...
            for action in actions:
                Utility_Function.remove_action_from_file(action, cleanup=True) 
...
def Insert_Start_End(action, start, end, free_handle=True):
    ...
    for fc in action.fcurves:
        ...
        kf_start = fc.keyframe_points.insert(start_point[1], start_point[0])
        kf_end = fc.keyframe_points.insert(end_point[1], end_point[0])
...
def Delete_Outside_Frame_Range(action, start, end):
...
def Trim_Action(action, start, end, isInsertFrame=True, isOffset=False, isDelete=True):


    if isInsertFrame:
        Insert_Start_End(action, start, end, free_handle=True)

    if isDelete:
        Delete_Outside_Frame_Range(action, start, end)

    if isOffset:
        Offset_Action(action, 0, start, end)
...
def Time_Scale(action, keyframes, subframe, source_frame_range, target_frame_range):
    ...
    for kf in keyframes:
        if subframe:
            ...
            kf.co[0] = int(Remap(kf.co[0], source_frame_range, target_frame_range))
            kf.handle_left[0] = int(Remap(kf.handle_left[0], source_frame_range, target_frame_range))
            kf.handle_right[0] = int(Remap(kf.handle_right[0], source_frame_range, target_frame_range))


    for pm in action.pose_markers:
        pm.frame = int(Remap(pm.frame, source_frame_range, target_frame_range))





def Time_Scale_Mode(action, keyframes, subframe, Source, Target):

    if Target[0] == "REMAP":

        if Target[1] > Target[2] or Target[3]:
        ...
        if action.use_frame_range:
            action.frame_start = target_frame_range[0]
            action.frame_end = target_frame_range[1]
    ...
    Time_Scale(action, keyframes, subframe, Source, target_frame_range)



def Time_Scale_Advanced(action, subframe, limit, Source, Target):
    ...
    Time_Scale_Mode(action, keyframes, subframe, Source, Target)
